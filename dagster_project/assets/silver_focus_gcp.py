"""Silver GCP Asset — Bronze Iceberg GCP → Polars 정제, Tags 평탄화, CostUnit 파생."""
...
@asset(
    ...
    deps=["bronze_iceberg_gcp"],
    description=(
        "GCP Bronze Iceberg 테이블을 Polars로 읽어 Tags를 team/product/env 컬럼으로 "
        "평탄화하고 cost_unit_key를 파생한 뒤 Silver 테이블에 overwrite 적재한다."
    ),
    group_name="silver",
)
def silver_focus_gcp(
    context: AssetExecutionContext,
    iceberg_catalog: IcebergCatalogResource,
) -> None:
    """GCP Bronze → Silver 변환.
    ...
    context.log.info(f"[GCP] Read {len(df)} rows from {bronze_table_name}")
    ...
    context.log.info(f"[GCP] Filtered to {len(df)} rows for {month_str}")
    ...
    context.log.info(f"[GCP] Wrote {len(df)} rows to {silver_table_name}")
